...
int unknown_price_edge = 99999; 
int unknown_price_inside = 99999;
int map[6][3] = 
{
    {0, 0, 0},
    {0, 0, 0},
...
void CreatAllPoint(const char get[]) // 构建位置图
{
    ...
    for (int i = 1; i <= 4; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            list[i][j] = CreatPoint(i, j, (map[i][j] == -1) ? unknown_price : map[i][j]); // 位置+方块类型
...
void BuildGraph() // 根据位置图，建立有向无环图，生成临接表
{
    ...
    for (int i = 1; i <= 4; i++)
    {
        ...
        list[i][1].kid_deep = (i < 4) ? &list[i + 1][1] : nullptr;
...
void tryTakeP2(std::priority_queue<RobotStatus, std::vector<RobotStatus>, PointCompare> &s, const RobotStatus &this_point) // 尝试在当前位置进行取P2操作
{
    ...
    for (int dir = 0; dir < 4; dir++) // 遍历检查四个方向是否有P2格
    {
        ...
        if (getP2Index(next_point_x, next_point_y) == -1) 
        {
            std::cerr << "Error: P2方块未记录在案！" << std::endl;
            continue; // 严谨检查
        }
...
RobotStatus *FindShortestRoadRobotStatus()
{
    ...
    bestDp[0][1][0] = 0;
    bestDir[0][1][0] = 0;
    s.push(RobotStatus{0, 1, 0, 0, 0}); // 起点入列

    while (s.empty() == false) // Dijkstra算法
    {
        ...
        if ((this_point.x == 5 && (this_point.y == 0 || this_point.y == 2 )) && this_point.mask > 0) // 到达出口点且有P2，结束
        {
        ...
        if (this_point_handle->kid_left != nullptr) // 如果左/右/前方有节点，则进入下个节点
        {
...
void Observe(char (&get)[25], int x, int y, int dir)
{
    switch (dir)
    {
    case 0: // 前
        for (int i = 0; i < 3; ++i)
        {
            int nx = x + 1;
            int ny = y + i - 1;
            if (nx >= 5 || ny < 0 || ny >= 3) continue;
            map[nx][ny] = JudgePrice(get + 2 * ((x+1) * 3 + y+i-1));
        }
        break;
    ...
    case 1: // 左
        for (int i = 0; i < 3; ++i)
        {
            int nx = x + i - 1;
            int ny = y - 1;
            if (nx < 0 || nx >= 5 || ny < 0) continue;
            map[nx][ny] = JudgePrice(get + 2 * (x+i-1 * 3 + y-1));
        }
        break;
    ...
    case 2: // 后
        for (int i = 0; i < 3; ++i)
        {
            ...
            map[nx][ny] = JudgePrice(get + 2 * (nx * 3 + ny));
        }
        break;
...
void ChangeMapPrice()
{
    ...
    unknown_price_edge = (P1_PRICE * (3 - num_P1))+ (P2_PRICE * (4 - num_P2)) + (PF_PRICE * (1 - num_PF))/2 ;
}
void RobotStep(char (&get)[25])
{
    if (!(std::cin >> get))
        return;
